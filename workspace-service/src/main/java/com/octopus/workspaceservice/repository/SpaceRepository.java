...
public interface SpaceRepository extends JpaRepository<Space, Integer> {
    ...
    @Query("SELECT s from Space s INNER JOIN Columns c ON s.id=c.space.id where day(s.createTime.getTime()) = ?1 and month(s.createTime.getTime()) = ?2 and year(s.createTime.getTime()) = ?3")
